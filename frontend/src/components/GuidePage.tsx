...
export default function GuidePage() {
  ...
  const fetchStep = async (state: string) => {
    setLoading(true)
    setError('')
    try {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 animate-fade-in">
      ...
      {error && (
        <div className="status-ineligible animate-slide-up">⚠️ {error}</div>
      )}

      {guide && !loading && (
        <div className="animate-slide-up" id="guide-content">
          {/* Progress Bar */}
          <div className="mb-6">
            <div className="flex justify-between text-sm text-gray-600 mb-2">
              <span>{t('guide.step', { current: guide.step_number, total: guide.total_steps })}</span>
          ...
          {guide.links.length > 0 && (
            <div className="card mb-6" id="guide-links">
              <h3 className="font-semibold text-gray-900 mb-3">🔗 {t('guide.useful_links')}</h3>
          ...
          <div className="flex gap-4">
            ...
            {guide.current_state === 'COMPLETE' && (
              <div className="w-full text-center py-4">
                <p className="text-xl font-bold text-green-700">🎉 {t('guide.complete')}</p>
              </div>
            )}
          </div>
        </div>
