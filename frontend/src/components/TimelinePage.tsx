...
export default function TimelinePage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 animate-fade-in">
      ...
      {error && (
        <div className="status-ineligible animate-slide-up" id="timeline-error" role="alert">
          <span aria-hidden="true">⚠️</span> {error}
        </div>
      )}

      {timelines.length > 0 && (
        <div className="space-y-6" id="timeline-results" role="status" aria-live="polite">
          {timelines.map((timeline, idx) => (
            ...
            >
              <h3 className="font-bold text-lg text-primary-900 mb-4">
                <span aria-hidden="true">📍</span> {timeline.constituency_id}
              </h3>
